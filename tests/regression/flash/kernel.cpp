...
// Tensor core path fixed to 8x8 tiles (fp16 inputs / fp32 accumulate).
...
static void flashattention_tcu(kernel_arg_t *arg) {
    ...
    auto block_size_c = arg->block_size_c; // expected 8 for TCU path

    // Fallback to SIMT if the tile sizes don't match the tensor-core config.
    if (block_size_r != tcu_ctx::tileM || block_size_c != tcu_ctx::tileN || head_dim != tcu_ctx::tileK) {
    ...
    size_t qk_fp16_elems = (block_size_r + block_size_c) * TCU_K;
    size_t p_fp16_elems  = block_size_r * TCU_K;
    size_t v_fp16_elems  = head_dim * TCU_K;
    size_t scores_elems  = block_size_r * block_size_c;
    ...
    auto local_K  = local_Q + block_size_r * TCU_K;
    auto local_P  = local_K + block_size_c * TCU_K;
    ...
    // Load Q_i into shared memory, pad to TCU_K
    for (uint32_t col = 0; col < TCU_K; ++col) {
        if (col < head_dim)
            local_Q[l_row * TCU_K + col] = float_to_fp16(Q_ptr[g_row_offset + col]);
        else
            local_Q[l_row * TCU_K + col] = 0;
    }
    ...
    for (uint32_t j = 0; j < seq_len; j += block_size_c) {
        ...
        // Load K_j and V_j^T into shared memory (col-major fp16, padded to tileK)
        for (uint32_t k = 0; k < block_size_c / block_size_r; ++k) {
          auto row = k * block_size_r + l_row; // K/V row within the tile
          auto row_offset = row * head_dim;
          for (uint32_t col = 0; col < TCU_K; ++col) {
            auto offset = col * TCU_K + row;
            if (col < head_dim) {
        ...
        // Pad remaining K/V rows to zero for tileK (rows >= block_size_c)
        if (l_row == 0) {
          for (uint32_t row = block_size_c; row < TCU_K; ++row) {
            for (uint32_t col = 0; col < TCU_K; ++col) {
              auto offset = col * TCU_K + row;
              local_K[offset]  = 0;
              local_Vh[offset] = 0;
        ...
        // Compute S_ij = Q_i · K_j^T using tensor cores (8x8 tile)
        ...
        float sp_buf[BLOCK_SIZE_C_MAX];
        for (uint32_t k = 0; k < block_size_c; ++k)
          sp_buf[k] = local_S[l_row * block_size_c + k];
        ...
        // Build P tile in fp16 (row-major, padded to TCU_K)
        for (uint32_t col = 0; col < TCU_K; ++col) {
          uint16_t p_val = 0;
          if (col < block_size_c)
            p_val = float_to_fp16(sp_buf[col]);
          local_P[l_row * TCU_K + col] = p_val;
        }

        __syncthreads();

        // Compute PV using tensor cores
        tcu_ctx::fill_fragment(fragO, 0);
        tcu_ctx::load_matrix_sync(fragP, local_P, TCU_K);
        tcu_ctx::load_matrix_sync<vt::col_major>(fragV, local_Vh, TCU_K);
        tcu_ctx::mma_sync(fragO, fragP, fragV, fragO);

        // Store PV result to shared (reuse local_S buffer)
        tcu_ctx::store_matrix_sync(local_S, fragO, head_dim);

        __syncthreads();

        // Update O with streaming softmax combine (unnormalized, divide once at end)
        for (uint32_t k = 0; k < head_dim; ++k) {
          float dot = local_S[l_row * head_dim + k];
          float old_contrib = old_weight * O_buf[k];
          float new_contrib = new_weight * dot;
          O_buf[k] = old_contrib + new_contrib;
        }
