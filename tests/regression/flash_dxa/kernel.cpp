...
__kernel void kernel_main(kernel_arg_t* __UNIFORM__ arg) {
    ...
    // K and V each get 2 ping-pong slots for DXA double buffering
    auto local_K = (TYPE*)local_Q + block_size_r * head_dim;
    auto local_V = (TYPE*)local_K + 2 * block_size_c * head_dim_tile;
    auto local_O = (TYPE*)local_V + 2 * block_size_c * head_dim_tile;
    auto local_P = (TYPE*)local_O + block_size_r * head_dim;

    TYPE* local_K_buf[2] = { local_K, local_K + block_size_c * head_dim_tile };
    TYPE* local_V_buf[2] = { local_V, local_V + block_size_c * head_dim_tile };

    // Transaction barriers: bar[0]/bar[1] track ping-pong stage 0/1's DXA
    // completion + CTA sync
    vortex::barrier bar[2] = { vortex::barrier(0), vortex::barrier(1) };
    // Only the CTA's first warp issues DXA commands; every warp waits.
    ...
    for (uint32_t j = 0; j < seq_len; j += block_size_c) {
        ...
        // S = QK^T (K tiles double-buffered via DXA: the next tile's
        // async copy is issued before this tile's compute)
        {
            uint32_t cur = 0;
            if (is_dxa_warp) {
                bar[0].expect_tx(1);
                vx_dxa_issue_2d_wg(kDescK, bar[0].id(), local_K_buf[0], /*coord0=*/0, /*coord1=*/j);
            }
            for (uint32_t h = 0; h < head_dim; h += head_dim_tile) {
                uint32_t tile_w = std::min(head_dim_tile, head_dim - h);
                uint32_t next_h = h + head_dim_tile;
                uint32_t nxt = cur ^ 1u;
                if (next_h < head_dim && is_dxa_warp) {
                    bar[nxt].expect_tx(1);
                    vx_dxa_issue_2d_wg(kDescK, bar[nxt].id(), local_K_buf[nxt], next_h, j);
                }
                bar[cur].arrive_and_wait();
                for (uint32_t i = 0; i < sp_count; ++i) {
                    uint32_t c = threadIdx.x + i * num_threads;
                    for (uint32_t e = 0; e < tile_w; ++e)
                        sp_buf[i] += Q_row[h + e] * local_K_buf[cur][c * head_dim_tile + e];
                }
                bar[cur].arrive_and_wait();
                cur = nxt;
            }
        ...
        // Update O (V tiles double-buffered via DXA)
        {
            uint32_t cur = 0;
            if (is_dxa_warp) {
                bar[0].expect_tx(1);
                vx_dxa_issue_2d_wg(kDescV, bar[0].id(), local_V_buf[0], /*coord0=*/0, /*coord1=*/j);
            }
            for (uint32_t h = 0; h < head_dim; h += head_dim_tile) {
                uint32_t tile_w = std::min(head_dim_tile, head_dim - h);
                uint32_t next_h = h + head_dim_tile;
                ...
                if (next_h < head_dim && is_dxa_warp) {
                    bar[nxt].expect_tx(1);
                    vx_dxa_issue_2d_wg(kDescV, bar[nxt].id(), local_V_buf[nxt], next_h, j);
                }
                bar[cur].arrive_and_wait();
                for (uint32_t k = threadIdx.x; k < tile_w; k += num_threads) {
                    TYPE dot = TYPE(0);
                    for (uint32_t c = 0; c < block_size_c; ++c) {
                        dot += local_P[l_row * block_size_c + c] * local_V_buf[cur][c * head_dim_tile + k];
                    }
