...
int main(int argc, char *argv[]) {
  ...
  RT_CHECK(vx_mem_alloc(device, out_buf_size, VX_MEM_WRITE, &C_buffer));
  ...
  for (uint32_t i = 0; i < size_sq; ++i) {
    h_A[i] = static_cast<int8_t>(Comparator<int>::generate() % 256 - 128);
    h_B[i] = static_cast<int8_t>(Comparator<int>::generate() % 256 - 128);
  }
  ...
  RT_CHECK(vx_copy_from_dev(h_C.data(), C_buffer, 0, out_buf_size));
